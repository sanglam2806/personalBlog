...
const CommentForm = ({slug}) => {
  ...
  return (
    <div className='bg-white bg-opacity-70 shadow-lg rounded-lg p-8 pb-12 mb-8'>
      <h1 className='text-xl mb-8 font-semibold border-b pb-4'>CommentForms</h1>
      ...
      <div className='grid grid-cols-1 gap-4 mb-4'>
        <div>
          ...
          <label className='ml-2 text-gray-500 cursor-pointer' htmlFor='storeData'>Save email and name for the next time?</label>
        </div>
      </div>
      {error && <p className='p-2 text-xs text-red-500'>All field are required!</p>}
      <div>
        <button
          type='button'
        ...
        {showSuccessMessage && <span className='text-md float-right font-semibold mt-3 text-green-500'>Comment Submitted</span>}
      </div>
    </div>
  )
}
